...
conn.sendButton(m.chat, '~MemeIndo~', await (await fetch(url.image)).buffer(), [['Menu', '.menu',],['Next', `.meme`,],['Creator', `.owner`]], m)
}
...
let name = conn.getName(who)
  conn.sendButton(m.chat, `Nih memeindo`, botdate, pickRandom(memeindo), [['Next', `/meme`],['Donasi', `.donasi`],['Creator', `.owner`]], m)
}
handler.tags = ['internet']
handler.help = ['meme']
handler.command = /^(meme)$/i
